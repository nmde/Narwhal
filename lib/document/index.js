...
const DOMImplementation = require('../dom/implementation');
const StyleSheetList = require('../stylesheet/list');
const DocumentTimeline = require('./timeline');
const ParentNode = require('../parent/node');
const HTMLCollection = require('../html/collection');
const Location = require('../location');
const GlobalEventHandlers = require('../global/eventHandlers');
...
class Document extends Node {
  constructor(window) {
    ...
    // Link this.title to the title of the window's screen
    watch(this, 'title', (oldValue, newValue) => {
      window.screen.screen.title = newValue;
    });
   ...
   * @param  {ElementCreationOptions} options Used with custom elements
   * @return {Element}                        The new element
   */
  createElement (tagName, options) {
    // createElement(null) == createElement('null')
    const string = String(tagName).toLowerCase();
    const element = new Element(string, this);
    if (typeof options === 'object' && typeof options.is === 'string') {
